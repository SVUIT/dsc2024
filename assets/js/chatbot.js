document.addEventListener("DOMContentLoaded", function () {
  ...
  //Lấy userID.
  function getUserId() {
    const apiUrl = 'https://chatbot.svuit.org/connect';
  ...
  function handleDisconnect() {
    ...
    if (userId) {
      const sent = navigator.sendBeacon(`https://chatbot.svuit.org/disconnect?uid=${userId}`);
      if (sent) {
  ...
  const iconImage = document.createElement('img');
    iconImage.src = '/dsc2024/assets/images/chatbot.png';
  ...
  function showLoader() {
    ...
    const loaderImg = document.createElement('img');
      loaderImg.src = '/dsc2024/assets/images/loader.gif';
      loaderImg.style = `
        width: 55px;
        height: 57px;
        `;
  ...
  function sendToServer(userId, userMessage) {
    const apiUrl = `https://chatbot.svuit.org/client_event?uid=${userId}`;
